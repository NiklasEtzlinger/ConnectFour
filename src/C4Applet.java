...
public class C4Applet extends Applet {
  ...
  private static final long serialVersionUID = 6148533328803836800L;
  ...
  private final static String RED_PIECE_IMAGE_NAME = "res/redpiece.gif";
  private final static String BLACK_PIECE_IMAGE_NAME = "res/blackpiece.gif";
  private final static String BOARD_IMAGE_NAME = "res/board.jpg";
...
//------------------------------------------------
  ...
  public void init() {
    ...
    try {
      ...
      blackPieceImage = Toolkit.getDefaultToolkit().getImage(BLACK_PIECE_IMAGE_NAME);
      redPieceImage = Toolkit.getDefaultToolkit().getImage(RED_PIECE_IMAGE_NAME);
      boardImage = Toolkit.getDefaultToolkit().getImage(BOARD_IMAGE_NAME);
...
//----------------------------------------------
  ...
  public static void main(String[] args) {
    Frame f = new Frame("Connect Four");
    f.setSize(450, 390);
    f.setResizable(false);
    C4Applet applet = new C4Applet();
    applet.init();
    f.add(applet);
    f.setVisible(true);
...
class ImagePanel extends Panel implements GameEventListener {
  ...
  private static final long serialVersionUID = 452359080396621067L;
...
//----------------------------------------
  ...
  // the column in which the mouse is currently in, if -1, the mouse is in no
  // column
  int blackColumn = -1;
...
//----------------------------------------
  ...
  ImagePanel(GameMaster aGame, C4Board aBoard, Player computer, AsynchronousPlayer human, Image boardImage,
      Image redPieceImage, Image blackPieceImage) {
    super();
...
//----------------------------------------
  ...
  /**
   * the mouse has moved we want to track which column the player is in
   */
  ...
  /**
   * Set the column where the black tip is. If the column hasn't changed does
   * nothing, if the column has changed, draws the new tip, and or clears the old,
...
//----------------------------------------
  ...
  /**
   * Draw a red tip above the given column. Draws to offscreen, must call update
   * to see it onscreen.
   */
  private void drawRedTip(int column) {
    offscreenGraphics.setColor(Color.red);
    offscreenGraphics.fillOval((column * COLUMN_WIDTH) + (TIP_TOP_X + RED_TIP_OFFSET), TIP_TOP_Y, TIP_DIAMETER,
        TIP_DIAMETER);
  }

  /**
   * Draw a black tip above the given column. Draws to offscreen, must call update
   * to see it onscreen.
   */
  private void drawBlackTip(int column) {
    offscreenGraphics.setColor(Color.black);
    offscreenGraphics.fillOval((column * COLUMN_WIDTH) + (TIP_TOP_X + BLACK_TIP_OFFSET), TIP_TOP_Y, TIP_DIAMETER,
        TIP_DIAMETER);
  }
