...
//--------------------------------------------------------
  // instance variables
  private Vector<GameEventListener> listeners = new Vector<GameEventListener>(); // listeners to the state,
...
//--------------------------------------------------------
  ...
  /**
   * Creates new GameMaster. The PlayerArray should have at least 1 player in it.
   * The order the players occur in the array is the order in which the players
   * will move. If the array is empty, an ArrayOutOfBoundsException will be
   * thrown.
   */
...
//------------------------------------------------------
  ...
  /**
   * Clone the listener vector in case the list of listeners is modified while
   * informing the listeners. This means the enumeration enumerates a copy of the
   * list, and additions can be made to the list while informing listeners.
   */
  @SuppressWarnings("unchecked")
  private synchronized Enumeration<GameEventListener> enumerateListeners() {
    return ((Vector<GameEventListener>) listeners.clone()).elements();
  }

  private void notifyListenersMoveMade(Move aMove) {
    Enumeration<GameEventListener> e = enumerateListeners();
    while (e.hasMoreElements()) {
      e.nextElement().moveMade(aMove);
    }
  }
...
//------------------------------------------------------
  ...
  /**
   * Start the game. Tells the board to start, starts the game loop thread, then
   * notifies listeners.
   */
  ...
  /**
   * Stop the game. Stops the game thread, tells the board the game is stopped,
   * and then notifies listeners.
   */
  ...
  /**
   * Restart the game. Stops the current game loop, tells the board to restart,
   * starts a new game loop, and then notifies listeners.
   */
...
//--------------------------------------------------------
  ...
  /**
   * The game thread class does the work of making the players move. Exits after
   * stop is called.
   */
