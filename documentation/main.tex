...
\begin{table}[H]
  ...
  \begin{tabular}{r rrrrrr}
    ...
    \hline
    SUM & 10726 & 43760 & 8905 & 7773 & 41644 & 5764 \\
    \hline
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.7\textwidth]{PruningResult}
  \caption{Move counts across all six variants visualised. Q2 and Q5a (pure
           minimax) dominate; each pruning step reduces the count, with Q5b
           (deep alpha-beta + middle-out ordering) achieving the lowest counts.}
  \label{fig:pruning}
\end{figure}
\clearpage
...
The value returned to minimax is always the difference between the current
player's raw strength and the opponent's raw strength:
\[
  \texttt{getStrength}(\text{player}) =
      \text{strength}_{\text{player}} - \text{strength}_{\text{opponent}}.
\]
