...
//--------------------------------------
  ...
  private void initSlots() {
     ...
     * things, its starting position, and its slope. the starting position is writen
     * as (row,column), where row exists in the set {0,1,... numberOfRows - 1} and
     * column lies in the set {0,1, ...number of columns -1} the slope is written as
     * <delRow, delColumn> where delRow and delColumn exist in the set {-1,0,1}
     *
     *
     * a group of four is valid and not repeated if 1) its starting and ending
     * positions both lie inside the board. 2) the slope is one of <1,0>, <1,1>,
     * <0,1>, <-1, 1> ie it occupies the range 90 to -45 degrees.
     * 
     *
     * iterate for all possible groups of four, choose the valid ones that are not
     * repeated
     */
    int totalNumber = 0;
      ...
      for (int column = 0; column < NUMBER_OF_COLUMNS; column++) {
        for (int delRow = -1; delRow <= 1; delRow++) {
          for (int delColumn = -1; delColumn <= 1; delColumn++) {
            if ((((delRow == 1) && (delColumn == 0)) || ((delRow == 1) && (delColumn == 1))
                || ((delRow == 0) && (delColumn == 1)) || ((delRow == -1) && (delColumn == 1)))
                && (inbounds(row + (3 * delRow), column + (3 * delColumn))) && (inbounds(row, column))

            ) {
              C4Row newRow = new C4Row(getSlot(row, column), getSlot(row + delRow, column + delColumn),
                  getSlot(row + (2 * delRow), column + (2 * delColumn)),
                  getSlot(row + (3 * delRow), column + (3 * delColumn)), stats);
...
//--------------------------------------
  ...
  /**
   * Returns the number of slots in a given column. column should be in
  ...
  /**
   * Return wether or not the given row,column pair exists on the board. Row and
  ...
  /**
   * Access a slot by its row. the index should be equal to row *
   * NUMBER_OF_COLUMNS + column, where row and column both start at 0
   */
...
//-----------------------------------------
  ...
  /**
   * Return an object which can be interrogated to discover the current state of
   * the game
   */
  ...
  /**
   * Try and make a move. Returns wether or not the move attempt was successful.
   *
   * m.toInt() should equal the column the player wants to move in. starting at 0.
   */
  ...
  /**
   * Get the list of moves that are possible for the given player. For the C4Board
   * this always returns the same array for each player. Some of the moves may not
   * be legal, but the legal moves is a subset of the moves returned.
   */
  public Move[] getPossibleMoves(Player aPlayer) {
    /*
     * We want to avoid creating new objects when communicating what moves are
     * allowed. To solve this we simply always return the same array when asked what
     * moves are possible for a player. Some of these moves may cause move() to
     * return false.
     */
  ...
  /**
   * A board must be cloneable since a copy of it is sent to players when we ask
   * them for their move.
   */
  public Object clone() {
    C4Board clone = null;
